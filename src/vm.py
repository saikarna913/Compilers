        ...
        while self.running and self.current_frame.ip < len(instructions):
            ...
            try:
                ...
                if opcode == LOAD_CONST:
                    ...
                    try:
                        # If one operand is a string, convert the other to string for concatenation
                    ...
                    loop_start = self.current_frame.ip - 1
                    self.current_frame.locals['__loop_start__'] = loop_start
                    # Initialize or update loop counter if needed
                    if '__loop_counter__' not in self.current_frame.locals:
                        self.current_frame.locals['__loop_counter__'] = 0
                
                elif opcode == LOOP_END:
                    # Jump back to the start of the loop
                    if '__loop_start__' in self.current_frame.locals:
                        self.current_frame.ip = self.current_frame.locals['__loop_start__']
                        # Update loop counter
                        self.current_frame.locals['__loop_counter__'] += 1
                
                elif opcode == BREAK:
                    # Jump to the instruction after the loop end
                    if '__loop_end__' in self.current_frame.locals:
                        self.current_frame.ip = self.current_frame.locals['__loop_end__']
                ...
                elif opcode == FOR_SETUP:
                    # Set up a for loop
                    # instruction[1]: start value
                    # instruction[2]: end value
                    # instruction[3]: step value
                    # instruction[4]: counter variable name
                    # instruction[5]: loop end position
                    
                    start = self.current_frame.stack.pop() if instruction[1] is None else instruction[1]
                    end = self.current_frame.stack.pop() if instruction[2] is None else instruction[2]
                    step = self.current_frame.stack.pop() if instruction[3] is None else instruction[3]
                    counter_var = instruction[4]
                    loop_end = instruction[5]
                    
                    # Initialize counter
                    self.current_frame.locals[counter_var] = start
                    # Record loop information
                    self.current_frame.locals['__loop_start__'] = self.current_frame.ip
                    self.current_frame.locals['__loop_end__'] = loop_end
                    self.current_frame.locals['__for_end__'] = end
                    self.current_frame.locals['__for_step__'] = step
                    self.current_frame.locals['__for_counter__'] = counter_var
                
                elif opcode == FOR_ITER:
                    # Iterate the for loop
                    counter_var = self.current_frame.locals['__for_counter__']
                    end = self.current_frame.locals['__for_end__']
                    step = self.current_frame.locals['__for_step__']
                    
                    # Check if we should continue the loop
                    current = self.current_frame.locals[counter_var]
                    if (step > 0 and current >= end) or (step < 0 and current <= end):
                        # Loop finished, jump to end
                        self.current_frame.ip = self.current_frame.locals['__loop_end__']
                    else:
                        # Continue with the loop body
                        # The counter will be incremented at the end of the loop
                        pass
                
                elif opcode == FOR_ITER_END:
                    # End of for loop iteration, increment counter
                    counter_var = self.current_frame.locals['__for_counter__']
                    step = self.current_frame.locals['__for_step__']
                    
                    # Increment counter
                    self.current_frame.locals[counter_var] += step
...
def run_example():
    ...
    from ast_1 import Integer, BinOp, Token
